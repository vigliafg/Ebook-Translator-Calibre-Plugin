...
from ..utils import log, traceback_error, request, socks_proxy
...
class Base:
    ...
    def get_iso639_target_code(cls, lang):
        # Simplified: assumes lang code is enough or returns lang itself if mapped
        code = cls.get_target_code(lang)
        # simplistic conversion for common languages if code is complex, but mostly google uses 'it', 'fr' etc.
        return code
    ...
    def api_key_error_message(cls):
        return 'A correct key format "{}" is required.'.format(cls.api_key_hint)
    ...
    def translate(self, content):
        ...
        try:
            ...
            raise UnexpectedResult(
                'Can not parse returned response. Raw data: {}'.format('\n\n' + error_message))
