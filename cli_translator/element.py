...
class PageElement(Element):
    ...
    def add_translation(self, translation=None):
        ...
        for rid, element in enumerate(self.reserve_elements):
            ...
            translation = re.sub(
                pattern, lambda _: element, translation) # Removed xml_escape(pattern) as it might escape regex chars
...
class Extraction:
    ...
    def get_sorted_pages(self):
        # Assumes pages are tuples/objects with .data (etree) and .href (string)
        # simplified sorting if needed, or just return pages
        pages = []
        pattern = re.compile(r'\.(xhtml|html|htm|xml|xht)$')
        for page in self.pages:
             # adapted for object with data and href
             # if isinstance(page.data, etree._Element) and pattern.search(page.href):
             # For ebooklib, we might pass a wrapper or handle it differently
             # For now, assume page has .data as etree element
            if hasattr(page, 'data') and isinstance(page.data, etree._Element) \
        ...
        for page in self.get_sorted_pages():
            ...
            if body is not None:
                elements.extend(self.extract_elements(page.id, body, []))
                 ...
                 # fallback if no x:body (maybe html body)
                 body = page.data.find('.//body') # or similar
                 if body is not None:
                      elements.extend(self.extract_elements(page.id, body, []))
...
class ElementHandlerMerge(ElementHandler):
    # Omitted for brevity if not used, but good to have if user enables it later.
    # Included basic implementation
    def prepare_original(self, elements):
        return super().prepare_original(elements) # Placeholder, assuming not using merge for now
...
def get_element_handler(placeholder, separator, position_override=None):
    config = get_config()
